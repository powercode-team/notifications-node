...
export abstract class NotificationBaseRepo<Entity extends IEntity<Id>, Id extends PK = PK, UserId extends PK = PK>
  ...
  constructor(parent: Repository<Entity>) {
    super(parent.target, parent.manager, parent.queryRunner);
  }
  ...
  protected async createEntity(data: Partial<Entity>): Promise<Entity> {
    ...
    return this.findOneByOrFail({ id: result.identifiers[0].id });
  }
  ...
  protected async updateEntity(id: Id, data: Partial<Entity>): Promise<Entity> {
    ...
    return this.findOneByOrFail({ id: id as any });
  }

  protected async deleteEntity(criteria: Criteria<Id> | FindOptionsWhere<INotificationQueueEntity<Id>>): Promise<number> {
  ...
  protected async findForProcessing(transport: string, limit?: number): Promise<Entity[]> {
    return this.manager.transaction(async (em: EntityManager) => {
      ...
      return em.find<Entity>(this.target, { where: where as any, order: { nextSend: 'ASC' } as any });
    });
  }

  protected async markAsRead(criteria: Criteria<Id> | FindOptionsWhere<Entity>): Promise<number> {
